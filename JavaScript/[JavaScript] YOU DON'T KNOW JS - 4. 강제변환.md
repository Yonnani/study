...
- 두 피연산자에 대해 ToPrimitive 강제변환을 실시하는 것으로 시작, 어느 한쪽이라도 문자열이 아닐 경우 양쪽 모두 ToNumber로 강제변환하여 숫자값으로 만들어 비교함
...
- 그러나 `<` 비교 대상이 모두 문자열 값이면, 각 문자를 단순 어휘(Lexicographic, 알파벳 순서로) 비교함
  ...
  두 배열을 ToPrimitive로 강제변환하면 문자열이기 때문에 a, b는 숫자로 강제변환하지 않고 문자 단위로 비교함
...
- 다음 코드
  ...
  a는 "4, 2"로, b는 "0, 4, 3"으로 문자열화시킨 후 어휘 비교를 함
...
- 다른 예제
  ...
  - a도 `[object Object]`, b도 `[object Object]`로 변환되어 어휘적인 비교 불가능함
  ...
  - `a < = b`는 실제로 `b <  a`의 평가 결과를 부정하도록 명세에 기술되어 있음
  - 그래서 `b < a`가 false이므로 `a <= b`는 이를 부정한 true가 됨
  - 자바스크립트 엔진은 `<=`를 '더 크지 않은'의 의미로 해석함
  - 더구나 `a >= b`는 먼저 `b <= a` 로 재해석한 다음 동일한 추론을 적용함

- 동등 비교에 관한 한 '엄격한 관계 비교(Strict Relational Comparison)'은 없음

  - 비교 전 a와 b 모두 명시적으로 동일한 타입임을 확실히 하는 방법 말고 `a < b` 같은 관계 비교 과정에서 암시적 강제변환을 원천 봉쇄할 수는 없음

- 강제변환이 유용하고 안전한 관계 비교라면 그냥 쓰자

- 조심해서 관계 비교를 해야할 것 같은 상황에서는`<`(또는 `>`)를 사용하기 전, 비교할 값들을 명시적으로 강제변환해두는 편이 안전함
...
- 강제변환은 많은 욕을 먹고 있지만 알고 보면 유용한 기능임
- 명시적 강제변환은 다른 타입의 값으로 변환하는 의도가 확실한 코드를 말하며 혼동의 여지를 줄이고 코드 가독성 및 유지 보수성을 높일 수 있는 장점이 있음
- 암시적인 강제변환은 '숨겨진' 로직에 의한 부수 효과가 있으며 타입변환이 처리되는 과정이 명확하지 않음
  - 암시적 강제변환이 코드 가독성을 향상시키는 장점도 있음
- 암시적 강제변환은 변환 과정이 구체적으로 어떻게 일어나는지 명확하게 알고 사용해야 함
